...
public class PlayerUpgrade
{
    ...
    {
        // Ellenőrzi, hogy az upgrade paraméter null-e
        if (upgrade == null)
        {
            // Hibaüzenet jelenik meg, ha a paraméter null
            Debug.LogError("PlayerUpgrade object is null in UpgradeData constructor.");
            return; // Kilép a konstruktorból
        }

        // Inicializálja a PlayerUpgrade mezőit a PlayerUpgradeData mezőiből
        ID = upgrade.ID;
        ...
        // A PlayerUpgrade példány nem ideiglenes másolatként kerül létrehozásra
        isTempCopy = false;

        // Inicializálja a módosítók listáját
        modifiers = new List<StatModifierData>();

        // Végigmegy az UpgradeData módosítóin
        foreach (var modifier in upgrade.modifiers)
        {
            // Lemásolja az egyes módosítókat egy új StatModifierData objektumba, és hozzáadja a listához
            modifiers.Add(new StatModifierData
            {
                type = modifier.type, // Beállítja a típusát
                baseValue = modifier.baseValue, // Beállítja az alapértékét
                scaleFactor = modifier.scaleFactor // Beállítja a skálázási tényezőt
            });
        }
    }
